...
function getCategoryVariant(category: string): 'success' | 'info' | 'warning' | 'default' {
  const incomeHighlight: string[] = ['Gaji', 'Bonus', 'THR']
  if (incomeHighlight.includes(category)) return 'success'
  if (category === 'Investasi') return 'info'
  if (category === 'Penjualan') return 'warning'
  return 'default'
}
...
export default function PenerimaanPage() {
  ...
  const openEditModal = useCallback((tx: Transaction) => {
    ...
    setForm({
      ...
      source: tx.source || INCOME_SOURCES[0],
  ...
  return (
    <div className="space-y-6">
      ...
      {filteredTransactions.length > 0 ? (
      ...
      ) : allPenerimaan.length === 0 ? (
        /* ── Empty State (no data at all) ────────────────────────────────── */
        <EmptyState
          title="Belum Ada Penerimaan"
          description="Catat pemasukan pertama Anda dengan menekan tombol Tambah Penerimaan."
          action={
            <Button size="sm" onClick={openAddModal}>
              <span className="flex items-center gap-1.5">
                <PlusIcon />
        ...
        <EmptyState
          ...
          description="Tidak ada penerimaan yang sesuai dengan filter yang dipilih."
        />
      )}
      ...
      <Modal
        isOpen={isModalOpen}
        onClose={closeModal}
        title={editingId ? 'Edit Penerimaan' : 'Tambah Penerimaan'}
      >
      ...
      <ConfirmDialog
        ...
        message="Apakah Anda yakin ingin menghapus penerimaan ini? Tindakan ini tidak dapat dibatalkan."
        confirmLabel="Hapus"
      />
    </div>
  )
